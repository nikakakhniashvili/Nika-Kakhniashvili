...
final class CartViewModel {
    ...
    func addProduct(product: Product?) {
        ...
        if let index = selectedProducts.firstIndex(where: { $0.id == product.id }),
           let selectedQuantity = selectedProducts[index].selectedQuantity,
           let productQuantity = product.selectedQuantity {
            selectedProducts[index].selectedQuantity = selectedQuantity + productQuantity
        } else {
